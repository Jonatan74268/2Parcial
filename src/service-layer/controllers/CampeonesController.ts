...
import AddCarTask, { AddCampeonData } from '../tasks/AddCampeonTask';
...
export default class CampeonesController extends BaseController {
  ...
  protected configureRouter(): void {
    this.router.get('/', this.getCampeonesList);
    this.router.get('/:id', this.findCampeon);
    this.router.post('/', this.addCampeon);
    this.router.put('/', this.updateCampeon);
    this.router.delete('/:id', this.deleteCampeon);
  }
  ...
  private async addCampeon(req: Request, res: Response): Promise<void> {
    try {
      ...
      const addCampeonTask = new AddCarTask(campeonData);
